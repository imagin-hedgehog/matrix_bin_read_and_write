...
int main(void)
{
    ...
    a) Информация о матрице - 2 числа типа int
            1)num - общее количество элементов матрицы
            2)n - размерность(порядок матрицы)
    ...
    */
    ...
    По интерфесу Размер фала должен состоять
        a) 2*sizeof(int) - число байт под 2 целых числа
                        и
        b) num*sizeof(float)) - кол-во байт под элементы матрицы

    Если размер файла не равен их сумме,
    то произошла ошибка при записи в файл или он был повреждён
    */
    if (size_file != 2*sizeof(int) + num*sizeof(float))
    {
    ...
    //         // i    - индекс строки
    //         // j    -индекс столбца
    //         count += fread((p + i*n + j), sizeof(float), 1, fPtr);
    //     }
    // 2)
    for(i = 0; i < num; i++)
    {
    ...
    // Порядок квадратной матрицы - есть и его размерность
    printf("Порядок квадратной матрицы: %d\n", n);

    /*Количесвто прочитанных элементов*/
    // Здесь может иметься ввиду вообще число всех записей
    // Включая и 2 целых числа - информация о матрице
    // Тогда надо к сумме ПРОЧИТАННЫХ элементов матрицы count прибавить 2
    // count += 2;
